  ...
  return (
    <DashboardShell role="Tenant" nav={tenantNav} title={`Welcome back${user?.name ? `, ${user.name}` : ""}`} subtitle="Track your applications and discover new homes.">
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard label="Total applications" value={totalCount} icon={FileText} delay={0} />
        <StatCard label="Pending review" value={pendingCount} icon={Clock} delay={0.05} hint="Avg. response 18h" />
        <StatCard label="Approved" value={approvedCount} hint={approvedTitle} icon={CheckCircle2} delay={0.1} />
      </div>
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          {applications.length > 0 && (
            <Button asChild variant="ghost" size="sm">
              <Link to="/dashboard/tenant/applications">View all</Link>
            </Button>
          )}
        </div>
        {recent.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border bg-card/50 p-10 text-center">
            ...
            <Button asChild className="mt-5 rounded-full"><Link to="/properties">Browse properties</Link></Button>
          </div>
        ) : (
                ...
                {recent.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell className="font-medium">
                      <Link to={`/properties/${r.propertyId}`} className="hover:underline">{r.propertyTitle}</Link>
                    </TableCell>
