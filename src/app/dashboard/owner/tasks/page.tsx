...
const OwnerTasksPage = () => {
  ...
  const submit = () => {
    ...
    const assigned = caretakers.find((c) => c.id === assignee);
    addItem({
      id: `task-${Date.now()}`,
      title: title.trim(),
      ...
      priority,
      status: "open",
      assignedTo: assigned?.name ?? "Care taker",
      createdBy: "owner",
    ...
    toast.success("Task created", {
      description: assigned ? `Assigned to ${assigned.name}.` : "Task added.",
    });
    setTitle("");
    setNotes("");
    setOpen(false);
  ...
  return (
    <DashboardShell
      ...
      subtitle="Oversee maintenance across your portfolio."
    >
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {visible.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border bg-card/50 p-10 text-center md:col-span-3">
            ...
            <h3 className="mt-3 font-display text-xl">Nothing in this view</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              {filter === "done"
                ? "Completed tasks will land here."
                : "Assign a task to trigger work across your portfolio."}
            </p>
          </div>
        ) : (
          visible.map((task) => (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Create work and hand it to one of your caretakers.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="e.g. Repair standby pump"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                rows={3}
                placeholder="Vendor details or instructions…"
              />
            </div>
          </div>
          <DialogFooter>
