...
import { ArrowLeft, Bath, BedDouble, Check, MapPin, Maximize2, Send } from "lucide-react";
...
const PropertyDetails = () => {
  ...
  const alreadyPending =
    user?.role === "tenant" && property ? hasPendingApplication(user.email, property.id) : false;
  ...
  const handleApply = (e: React.FormEvent) => {
    ...
    if (!user) {
      ...
      router.push(`/auth?from=/properties/${property.id}`);
      return;
    }
    if (user.role !== "tenant") {
      toast({
          ...
          <aside className="lg:col-span-4">
            <div className="sticky top-24 rounded-2xl border border-border bg-card p-6 shadow-elevated">
              ...
              <div className="mt-6 space-y-3 border-t border-border pt-6 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Caretakers</span>
                  <span className="font-medium">{property.caretakerIds.length}</span>
                </div>
              </div>
            </div>
          </aside>
